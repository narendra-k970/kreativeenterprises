...
class Elementor_Gallery_Widget extends \Elementor\Widget_Base {
  ...
  public function get_script_depends() {
    if (\Elementor\Plugin::$instance->editor->is_edit_mode() || \Elementor\Plugin::$instance->preview->is_preview_mode()) {
      return ['isotope', 'swiper-6.1.1', 'gallery-handler'];
    }
  ...
  protected function render_items() {
    ...
    foreach ($this->settings['gallery'] as $index => $item) {
      ...
      <?php if ($this->settings['navigation'] != 'none' && $this->settings['count_items'] > 0 && count($this->settings['gallery']) > $this->settings['count_items'] && $this->settings['count_items'] + $key2 == $index): ?>
        </div>
        <div class="load-items load-items<?php echo esc_html($i1); ?>">
      <?php $key2 = $index;
      $i1++;
        ...
        <div <?php echo $this->get_render_attribute_string('item-wrapper') ?>>
          <div class="wrap">
            ...
            <?php endif; ?>
            <a href="#" data-popup-json="<?php echo esc_attr(json_encode($popup_array)) ?>" data-id="<?php echo esc_attr($index) ?>"></a>
          </div>
				</div>
			<?php } ?>
