...
class Elementor_Banner_Widget extends \Elementor\Widget_Base {
  ...
  public function get_script_depends() {
    if (\Elementor\Plugin::$instance->editor->is_edit_mode() || \Elementor\Plugin::$instance->preview->is_preview_mode()) {
      return ['banner-handler', 'gsap', 'pt-youtube-video', 'pt-categories', 'owl-carousel', 'swiper-6.1.1', 'typed'];
    }

    $scripts = ['banner-handler', 'gsap', 'pt-categories', 'owl-carousel', 'swiper-6.1.1', 'typed'];
    ...
    $repeater->add_control(
      ...
      [
        ...
        'options' => [
          ...
          'number' => esc_html__('With serial number', 'pt-addons'),
          'decor-line' => esc_html__('Decor line', 'pt-addons'),
        ],
      ]
    );
  ...
  public function banner_items() {
    ...
    foreach ($settings['banner_items_list'] as $index => $item):
      ...
      if (!empty($item['video_url']) && $item['play_in_lightbox'] == 'on') {
        ...
        $video = Embed::get_embed_html($item['video_url'], $embed_params, $embed_options);
        $play_button_html = '<div class="play-button-block popup-gallery images"><a href="#" data-type="video" data-size="1280x768" data-video=\'<div class="wrapper"><div class="video-wrapper">' . $video . '</div></div>\'><i class="music-and-multimedia-play-button"></i></a></div>';
      }
